<?php
/**
 * Contoh Script Bridge API Starshort via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Starshort di sistem Bridge
function callStarshortAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/starshort/api/v1";

// Contoh 1: Fitur Cari Film Starshort
echo "\n=====================[ 1. Pencarian Starshort ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callStarshortAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callStarshortAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callStarshortAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/starshort ◀◀◀\n\n";
